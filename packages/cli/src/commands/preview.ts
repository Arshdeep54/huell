...
import { migrateDocs } from "@doctor/docs-core";

// Resolves the docs-site template this CLI ships against. Inside this
// monorepo (dev, or `pnpm --filter doctor-docs dev`) that's the real
// templates/docs-site workspace package, already `pnpm install`-ed.
//
// TODO(publish): a standalone `npm install -g doctor-docs` doesn't have this
...
function resolveTemplateDir(): string {
  ...
  throw new Error(
    "Could not find the docs-site template. This build of doctor-docs doesn't yet bundle it standalone — " +
      "run it from within the Doctor monorepo (pnpm --filter doctor-docs dev preview) for now.",
  );
}
...
export function runPreview(targetDir: string) {
  ...
  if (!existsSync(path.join(docsDir, "docs.json"))) {
    console.error(`No docs.json found in ${docsDir}. Run "doctor-docs init" first.`);
    process.exitCode = 1;
    return;
  }
