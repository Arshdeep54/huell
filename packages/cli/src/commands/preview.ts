...
// monorepo (dev, or `pnpm --filter huell-docs dev`) that's the real
// templates/docs-site workspace package, already `pnpm install`-ed.
//
// TODO(publish): a standalone `npm install -g huell-docs` doesn't have this
...
function resolveTemplateDir(): string {
  ...
  throw new Error(
    "Could not find the docs-site template. This build of huell-docs doesn't yet bundle it standalone — " +
      "run it from within the Huell monorepo (pnpm --filter huell-docs dev preview) for now.",
  );
}
...
export function runPreview(targetDir: string) {
  ...
  if (!existsSync(path.join(docsDir, "docs.json"))) {
    console.error(`No docs.json found in ${docsDir}. Run "huell-docs init" first.`);
    process.exitCode = 1;
    return;
  }
