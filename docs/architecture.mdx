...
That check is a single route (simplified below — the actual implementation lives at `apps/web/src/app/api/caddy/ask-domain/route.ts`):

```ts
export async function GET(request) {
  const domain = request.nextUrl.searchParams.get("domain");
  const slug = domain.replace(`.docs.${orgDomain}`, "");
  const project = db.select().from(projects).where(eq(projects.slug, slug)).get();
  return project ? new Response("OK", { status: 200 }) : new Response("Forbidden", { status: 403 });
}
```

Everything that decides whether a hostname is legitimate lives in that one route, in the app — Caddy itself stays dumb about projects, members, or anything else in the database.
